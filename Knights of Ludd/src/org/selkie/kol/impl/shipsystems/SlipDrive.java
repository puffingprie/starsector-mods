...
public class SlipDrive extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        for (int i = 0; i < (500 * Global.getCombatEngine().getElapsedInLastFrame()); i++) {
            FakeSmokePlugin.addFakeSmoke(MathUtils.getRandomNumberInRange(0.38f, 0.67f), MathUtils.getRandomNumberInRange(70f, 90f),
    ...
    public void unapply(MutableShipStatsAPI stats, String id) {
        ...
        if (ship.getSystem().getEffectLevel() <= 0f) {
            ...
            for (int i = 0; i < 100; i++) {
                FakeSmokePlugin.addFakeSmoke(MathUtils.getRandomNumberInRange(0.1f, 0.78f), MathUtils.getRandomNumberInRange(50f, 90f),
    ...
    public StatusData getStatusData(int index, State state, float effectLevel) {
        if (index == 0) {
            return new StatusData("slipping into phasespace...", false);
        }
        return null;
    }
}
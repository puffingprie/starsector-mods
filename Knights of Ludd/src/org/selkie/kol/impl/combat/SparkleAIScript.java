...
public class SparkleAIScript implements MissileAIPlugin {
    ...
    protected SharedAIData data;

    public SparkleAIScript(MissileAPI missile) {
        this.missile = missile;
        r = (float) Math.random();
        elapsed = -(float) Math.random() * 0.5f;

        data = SparkleHullMod.getSharedData(missile.getSource());
...
//			}
    ...
    protected void acquireNewTargetIfNeeded() {
        ...
        float maxDistFromSourceShip = SparkleHullMod.MAX_DIST_FROM_SOURCE_TO_ENGAGE_AS_PD;
        float maxDistFromAttractor = SparkleHullMod.MAX_DIST_FROM_ATTRACTOR_TO_ENGAGE_AS_PD;
    ...
    protected int getNumMotesTargeting(CombatEntityAPI other) {
        ...
        for (MissileAPI mote : data.drones) {
            ...
            if (mote.getUnwrappedMissileAI() instanceof SparkleAIScript) {
                SparkleAIScript ai = (SparkleAIScript) mote.getUnwrappedMissileAI();
